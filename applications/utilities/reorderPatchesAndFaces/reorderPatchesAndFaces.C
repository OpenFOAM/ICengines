...
Description
    Utility to reorder the patches of a case, along with correct face index
...
int main(int argc, char *argv[])
{
    ...
    forAll(referencePatchNames, patchi)
    {
        ...
        {
            FatalErrorInFunction
                << "Cannot find reference patch "
                << referencePatchNames[patchi]
    ...
    // get reference to patches
    const polyBoundaryMesh& oldPatches = mesh.poly().boundary();

    polyTopoChange meshMod(mesh);

    forAll(oldPatches, newPatchi)
    {
        // get old patch index
        const label oldPatchi = newToOldPatches[newPatchi];

        // get reference to old patch
