...
License
    ...
    MOST OF THE CODE BELOW is originally published by OpenCFD:
...
#include "truncatedCone_searchableSurface.H"
...
Foam::searchableSurfaces::truncatedCone::coordinates() const
{
...
void Foam::searchableSurfaces::truncatedCone::boundingSpheres
(
    pointField& centres,
...
Foam::tmp<Foam::pointField> Foam::searchableSurfaces::truncatedCone::points() const
{
...
void Foam::searchableSurfaces::truncatedCone::findNearestAndNormal
(
    const point& sample,
...
) const
{
    ...
    // Nearest and normal on truncatedCone. Initialise to far-away point so if not
    // set it picks one of the disk points
    point neartruncatedCone(GREAT, GREAT, GREAT);
    vector normaltruncatedCone(1, 0, 0);

    // Nearest and normal on inner truncatedCone. Initialise to far-away point
    point iCneartruncatedCone(GREAT, GREAT, GREAT);
    vector iCnormaltruncatedCone(1, 0, 0);
    ...
    if (mag(p1) > ROOTVSMALL)
    {
        ...
        // Find vector along the two end of truncatedCone
        const vector b = normalised(projPt2 - projPt1);

        // Find the vector along sample pt and pt at one end of truncatedCone
        ...
        if (mag(a) <= ROOTVSMALL)
        {
            ...
            neartruncatedCone = (a & b)*b + projPt2;

            vector b1 = (p1 & b)*b;
            normaltruncatedCone = normalised(p1 - b1);
        }
        else
        {
            // Find nearest point on truncatedCone surface
            neartruncatedCone = (a & b)*b + projPt1;

            // Find projection along surface of truncatedCone
            vector b1 = (p1 & b)*b;
            normaltruncatedCone = normalised(p1 - b1);
        }
        ...
        if (innerRadius1_ > 0 || innerRadius2_ > 0)
        {
            ...
            if (mag(iCa) <= ROOTVSMALL)
            {
                ...
                // Find normal unitvector
                iCneartruncatedCone = (iCa & iCb)*iCb+iCprojPt2;

                vector b1 = (iCp1 & iCb)*iCb;
                iCnormaltruncatedCone = normalised(iCp1 - b1);
            }
            else
            {
                // Find nearest point on truncatedCone surface
                iCneartruncatedCone = (iCa & iCb)*iCb+iCprojPt1;
    ...
    // Select nearest out of the 4 points (outer truncatedCone, disk1, disk2, inner
    // truncatedCone)

    FixedList<scalar, 4> dist;
    dist[0] = magSqr(neartruncatedCone-sample);
    dist[1] = magSqr(disk1Point-sample);
    dist[2] = magSqr(disk2Point-sample);
    dist[3] = magSqr(iCneartruncatedCone-sample);
    ...
    if (minI == 0)  // Outer truncatedCone
    {
        // Closest to (infinite) outer truncatedCone. See if needs clipping to end disks

        {
            vector v1(neartruncatedCone-point1_);
            ...
            {
                // Near point 1. Set point to intersection of disk and truncatedCone.
                // Keep normal from truncatedCone.
                neartruncatedCone = disk1Point;
            }
            else if (para < 0.0 && magV1 < radius1_)
            {
                // On disk1
                neartruncatedCone = disk1Point;
                normaltruncatedCone = disk1Normal;
            }
            else if (para > magDir_ && magV1 >= radius2_)
            {
                // Near point 2. Set point to intersection of disk and truncatedCone.
                // Keep normal from truncatedCone.
                neartruncatedCone = disk2Point;
            }
            else if (para > magDir_ && magV1 < radius2_)
            {
                // On disk2
                neartruncatedCone = disk2Point;
                normaltruncatedCone = disk2Normal;
            }
        }
        info.setPoint(neartruncatedCone);
        nearNormal = normaltruncatedCone;
    }
    ...
    else                // Near to inner truncatedCone
    {
        {
            vector v1(iCneartruncatedCone-point1_);
            scalar para = (v1 & unitDir_);
            // Remove the parallel component and normalise
            v1 -= para*unitDir_;
            ...
            {
               iCneartruncatedCone = disk1Point;
            }
            else if (para < 0.0 && magV1 < innerRadius1_)
            {
               iCneartruncatedCone = disk1Point;
               iCnormaltruncatedCone = disk1Normal;
            }
            else if (para > magDir_ && magV1 >= innerRadius2_)
            {
               iCneartruncatedCone = disk2Point;
            }
            else if (para > magDir_ && magV1 < innerRadius2_)
            {
               iCneartruncatedCone = disk2Point;
               iCnormaltruncatedCone = disk2Normal;
            }
        }
        info.setPoint(iCneartruncatedCone);
        nearNormal = iCnormaltruncatedCone;
    }
...
Foam::scalar Foam::searchableSurfaces::truncatedCone::radius2
(
    const truncatedCone& truncatedCone,
    const point& pt
)
{
    const vector x = (pt-truncatedCone.point1_) ^ truncatedCone.unitDir_;
    return x&x;
}


// From mrl.nyu.edu/~dzorin/rend05/lecture2.pdf,
// Ray Tracing II, Infinite truncatedCone ray intersection.
void Foam::searchableSurfaces::truncatedCone::findLineAll
(
    const truncatedCone& truncatedCone,
...
) const
{
    ...
    vector point1Start(start-truncatedCone.point1_);
    vector point2Start(start-truncatedCone.point2_);
    vector point1End(end-truncatedCone.point1_);

    // Quick rejection of complete vector outside endcaps
    scalar s1 = point1Start & (truncatedCone.unitDir_);
    scalar s2 = point1End & (truncatedCone.unitDir_);

    if ((s1 < 0.0 && s2 < 0.0) || (s1 > truncatedCone.magDir_ && s2 > truncatedCone.magDir_))
    {
        return;
    }
    ...
    scalar radius_sec = truncatedCone.radius1_;

    {
        ...
        {
            tPoint1 = -s1/s;
            tPoint2 = -(point2Start&(truncatedCone.unitDir_))/s;
            ...
            {
                scalar r2 = radius2(truncatedCone, start+tPoint1*V);
                vector p1 = (start+tPoint1*V-point1_);
                vector p2 = (start+tPoint1*V-point2_);
                radius_sec = truncatedCone.radius1_;
                scalar inC_radius_sec = innerRadius1_;

                if (mag(p2&(truncatedCone.unitDir_)) < mag(p1&(truncatedCone.unitDir_)))
                {
                    radius_sec = truncatedCone.radius2_;
            ...
            {
                // Decompose sample-point1 into normal and parallel component
                vector p1 = (start+tPoint2*V-truncatedCone.point1_);
                vector p2 = (start+tPoint2*V-truncatedCone.point2_);
                radius_sec = truncatedCone.radius1_;
                scalar inC_radius_sec = innerRadius1_;
                if (mag(p2&(truncatedCone.unitDir_)) < mag(p1&(truncatedCone.unitDir_)))
                {
                ...
                scalar r2 = radius2(truncatedCone, start+tPoint2*V);
                if (r2 <= sqr(radius_sec) && r2 >= sqr(inC_radius_sec))
                {
    ...
    scalar deltaRadius = truncatedCone.radius2_-truncatedCone.radius1_;
    if (mag(deltaRadius) <= ROOTVSMALL)
    {
        vector point1Start(start-truncatedCone.point1_);
        const vector x = point1Start ^ truncatedCone.unitDir_;
        const vector y = V ^ truncatedCone.unitDir_;
        const scalar d = sqr(0.5*(truncatedCone.radius1_ + truncatedCone.radius2_));
    ...
    {
        vector va = truncatedCone.unitDir_;
        vector v1 = normalised(end-start);
        scalar p  = (va&v1);
        vector a1 = (v1-p*va);

        // Determine the end point of the truncatedCone
        point pa =
            truncatedCone.unitDir_*truncatedCone.radius1_*mag(truncatedCone.point2_-truncatedCone.point1_)
           /(-deltaRadius)
           +truncatedCone.point1_;

        scalar l2 = sqr(deltaRadius)+sqr(truncatedCone.magDir_);
        scalar sqrCosAlpha = sqr(truncatedCone.magDir_)/l2;
...
void Foam::searchableSurfaces::truncatedCone::insertHit
(
    const point& start,
    const point& end,
...
Foam::boundBox Foam::searchableSurfaces::truncatedCone::calcBounds() const
{
    // Adapted from
...
Foam::searchableSurfaces::truncatedCone::truncatedCone
(
    const IOobject& io,
    const point& point1,
...
const Foam::wordList& Foam::searchableSurfaces::truncatedCone::regions() const
{
    if (regions_.empty())
    {
        regions_.setSize(1);
...
void Foam::searchableSurfaces::truncatedCone::findNearest
(
...
void Foam::searchableSurfaces::truncatedCone::findLine
(
...
) const
{
    ...
    if (innerRadius1_ > 0.0 || innerRadius2_ > 0.0)
    {
        ...
        truncatedCone innertruncatedCone
        (
            io,
            point1_,
            innerRadius1_,
...
void Foam::searchableSurfaces::truncatedCone::findLineAny
(
    const pointField& start,
    const pointField& end,
    List<pointIndexHit>& info
...
void Foam::searchableSurfaces::truncatedCone::getRegion
(
...
void Foam::searchableSurfaces::truncatedCone::getNormal
(
    const List<pointIndexHit>& info,
    vectorField& normal
) const
...
void Foam::searchableSurfaces::truncatedCone::getVolumeType
(
